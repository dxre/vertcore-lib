...
## URI Validation
The main use that we expect you'll have for the `URI` class in vertcore is validating and parsing bitcoin URIs. A `URI` instance exposes the address as a vertcore `Address` object and the amount in Satoshis, if present.
